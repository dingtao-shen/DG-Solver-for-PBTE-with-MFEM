...
namespace DGSolver {
    ...
    void PBTE_NonGraySMRT_MPI<dim>::exchange_solutions(const SpatialMesh::MeshPartitionInfo<dim>& partition_info) {
        ...
        for (const auto& kv : dest_to_cells) {
            ...
            if (sbuf.size() != buf_elems) {
                sbuf.assign(buf_elems, 0.0);
            }
            // Pack in parallel
            #pragma omp parallel for schedule(static)
            for (int i = 0; i < count; ++i) {
                const int cid = cell_ids[i];
                const int lidx = local_cell_idx.at(cid);
        ...
        // Process received data and scatter into nbr_solutions
        for (size_t idx_src = 0; idx_src < recv_sources.size(); ++idx_src) {
            const int source_rank = recv_sources[idx_src];
            ...
            if (expected_count * static_cast<size_t>(single_cell_msg_size) != data_buf.size()) {
                throw std::runtime_error("Received buffer size does not match expected neighbor count");
            }
            #pragma omp parallel for schedule(static)
                                        ...
                                        std::vector<std::vector<Eigen::MatrixXd>>& outflow)
    {
                    ...
                    for (int j1 = 0; j1 < CC.NPOLE; ++j1) {
                        for (int j2 = 0; j2 < CC.NAZIM; ++j2) {
                            ...
                            for (int loc = 0; loc < n_local_cells; ++loc) {
                                ...
                                for (int i = 0; i < dim; ++i) {
                                    Coeff = Coeff.array() - vg * (dir(i) * StfMat[CellID][i].array());
                                }
                                for (int il = 0; il < dim + 1; ++il) {
                                    const double f = outflow[j1][j2](il, CellID);
                                    Coeff = Coeff.array() + 0.5 * vg * (f + std::abs(f)) * MassOnFace[CellID][il].array();
                                }
        ...
        while(residual > CC.TOL && iter_step < CC.TMAX){
            ...
            double* local_Tc_ptr = local_Tc.data();
            ...
            #pragma omp parallel
            {
                ...
                #pragma omp for collapse(4) schedule(static) reduction(+:local_Tc_ptr[:local_Tc_size])
                for(int j1 = 0; j1 < CC.NPOLE; j1++){
                for(int j2 = 0; j2 < CC.NAZIM; j2++){
                for(int p = 0; p < CC.POLAR_DIM; p++){
                for(int s = 0; s < CC.NSPEC; s++){
                    ...
                    for(size_t k = 0; k < comp_order.size(); ++k){
                        ...
                        for(int il = 0; il < dim+1; il++){
                            ...
                            if(BCTAG == 0){
                                ...
                                if(local_cell_set.find(NBR) != local_cell_set.end()){
                                    ...
                                    rhs.noalias() -= 0.5 * vg * (f - abs(f)) * (FluxInt[CellID][il] * nbr_sol);
                                }
                        ...
                        // Accumulate Tc contribution for this (cell, p, s, j1, j2) using array-section reduction
                        ...
                        for (int d = 0; d < CC.DOF; ++d) {
                            local_Tc_ptr[tc_offset + static_cast<size_t>(d)] += scale * sol[d];
                        }
